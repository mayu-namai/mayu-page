          ...
          <div className="mt-16">
            ...
            <div className="text-xs min-[480px]:text-sm text-gray-500 leading-[1.9] space-y-5">
              ...
              <div className="my-6 space-y-6">
                {[
                  { src: "p1_d3_ch_ui.png",        captionJa: "キャラクタフレーム選択画面。キャラクタ領域は赤く表示される。", captionEn: "Character frame selection screen. The character region is highlighted in red." },
                  { src: "p1_d3_st_ui.png",         captionJa: "ステージフレーム選択画面。",     captionEn: "Stage frame selection screen." },
                  { src: "p1_d3_selection_ui.png",  captionJa: "確認画面。",                     captionEn: "Confirmation screen." },
              ...
              <p>{en
                ? "The character region is extracted from the selected character frame, and the stage frame is captioned to generate the foundation of the vignette illustration. The character material and the foundation are then composited to produce the final vignette illustration."
                : "選択されたキャラクタフレームからキャラクタ領域が切り抜かれ、ステージフレームはキャプション化されることで、ヴィネットイラストの土台が生成される。その後、キャラクタ素材と土台を組み合わせることで、最終的にヴィネットイラストが生成される。"}
              </p>
              <p>{en
                ? "Below are vignette illustrations generated by VigNette using Plants & Plants [1] as input."
                : "以下に、Plants & Planets [1] を入力としてVigNetteにより生成されたヴィネットイラストを示す。"}
              </p>
              <figure className="my-6">
                <img
                  src={`${bp}/p1_d4_frames.png`}
                  alt={en ? "Frame images from Plants & Planets" : "Plants & Planets のフレーム画像"}
                  className="w-full h-auto"
                />
                <figcaption className="text-xs text-gray-400 mt-2 leading-[1.7]">
                  {en
                    ? "Frame images from Plants & Planets. In Plants & Planets, two cats adventure together through space and plant cherry blossoms on a planet covered in gray sand."
                    : "Plants & Planets のフレーム画像。Plants & Planetsでは、2匹の猫がともに宇宙を冒険し、灰色の砂で覆われた星に桜を植える。"}
                </figcaption>
              </figure>
              <figure className="my-6">
                <img
                  src={`${bp}/p1_d4_outputs.png`}
                  alt={en ? "Generation results using Plants & Planets as input" : "Plants & Planets を入力とした場合の生成結果"}
                  className="w-full h-auto"
                />
                <figcaption className="text-xs text-gray-400 mt-2 leading-[1.7]">
                  {en
                    ? "Generation results using Plants & Planets as input. Outputs A–E are vignette illustrations generated in semiautomated mode, and Output X is generated in automated mode. Character A–E and Stage A–E are the character frames and stage frames used to generate Outputs A–E respectively. Similarly, Character X and Stage X are the frames used to generate Output X. In automated mode, three stage frames were extracted."
                    : "Plants & Planets を入力とした場合の生成結果。Output A–Eは半自動生成モード、Output Xは自動生成モードによって生成されたヴィネットイラストである。Character A–EおよびStage A–Eは、それぞれOutput A–Eの生成に用いたキャラクタフレームおよびステージフレームである。同様に、Character XおよびStage XはOutput Xの生成に用いたフレームである。なお、自動生成モードでは3枚のステージフレームを抽出した。"}
                </figcaption>
              </figure>
              <p>
